...
public class Ball : MonoBehaviour
{
    ...
    private IEnumerator CatchAndShoot(Player player)
    {
        ...
        Vector2 direction = player.transform.position.x < 0 ? Vector2.right : Vector2.left;
        transform.localPosition = Vector2.zero + direction * .5f;

        yield return new WaitForSeconds(1f);

        transform.parent = player.transform.parent;

        _rigidbody2D.velocity = direction * _superSpeed;
...
//        if (isPlayer1)
//        {
//            _rigidbody2D.velocity = Vector2.one * _superSpeed;
//        }
//        else
//        {
//            _rigidbody2D.velocity = -Vector2.one * _superSpeed;
